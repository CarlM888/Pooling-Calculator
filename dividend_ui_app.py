...
if _opts:
    ...
    # Export scenario JSONst.markdown("---")
...
def exa_declared_dividend(net: float, pair_units: float, rules: 'PoolRules') -> float:
    if pair_units <= 0:
        return 0.0
...
try:
    if pool == 'EXA':
        ...
        with st.expander('📥 Import EXA Collation', expanded=False):
            txt_exa = st.text_area('Paste EXA collation dump', height=220, key='txt_exa')
        ...
        if st.button('Calculate EXA Approximates / WillPays', key='btn_exa_approxs'):
            try:
                ...
                for _, r in exa_df.iterrows():
                    ...
                    units = float(r['Units'])
                    div = exa_declared_dividend(net, units, rules)
        ...
        if st.button('Calculate EXA Declared Dividend', key='btn_exa_calc'):
            try:
                ...
                div = exa_declared_dividend(net, pair_units, rules)
...
try:
    if pool == "FFR":
        st.markdown("### FFR Collation Upload (First Four Runners)")
        col_up, _ = st.columns([2,1])
        with col_up:
            ffr_file = st.file_uploader("Upload FFR collation (.txt)", type=["txt"], key="ffr_upl")
            ffr_text = None
            if ffr_file is not None:
                ffr_text = ffr_file.read().decode("utf-8", errors="ignore")
                st.session_state['ffr_text'] = ffr_text
            elif 'ffr_text' in st.session_state:
                ffr_text = st.session_state['ffr_text']

        pool_total = _D(0); first_sp = second_sp = third_sp = fourth_sp = {}
        if ffr_text:
            pool_total, first_sp, second_sp, third_sp, fourth_sp = parse_ffr_collation(ffr_text)
            # Prefill Gross Sales from Pool Total (runs even if pandas isn't available)
            try:
                pt = float(pool_total) if pool_total is not None else 0.0
                gs = float(st.session_state.get('gross_sales', 0.0))
                if gs <= 0.0 and pt > 0.0:
                    st.session_state['gross_sales'] = pt
            except Exception:
                pass
            try:
                import pandas as pd
                df_ffr = pd.DataFrame([
                    {"Runner": k, "First": float(first_sp.get(k,0)), "Second": float(second_sp.get(k,0)),
                     "Third": float(third_sp.get(k,0)), "Fourth": float(fourth_sp.get(k,0))}
                    for k in sorted(set(first_sp) | set(second_sp) | set(third_sp) | set(fourth_sp))
                ])
                st.caption(f"Parsed FFR spread (Pool Total: ${pool_total})")
                if float(st.session_state.get('gross_sales', 0.0)) == float(pool_total):
                    st.caption(':memo: Autofilled Gross Sales from Pool Total')
                st.dataframe(df_ffr, hide_index=True, use_container_width=True)
            except Exception as _e:
                st.info(f"Parsed FFR spread. (table view skipped: {_e})")

        st.markdown("### Winners (ordered)")
        valid = sorted([r for r,v in (first_sp or {}).items() if v > 0]) or [1,2,3,4]
        a_def = valid[0]; b_def = valid[1] if len(valid)>1 else a_def+1
        c_def = valid[2] if len(valid)>2 else b_def+1
        d_def = valid[3] if len(valid)>3 else c_def+1

        cfa, cfb, cfc, cfd = st.columns(4)
        with cfa:
            ffr_first  = st.number_input('First (winner)', value=int(a_def), step=1, format='%d', key='ffr_sel_first')
        with cfb:
            ffr_second = st.number_input('Second', value=int(b_def), step=1, format='%d', key='ffr_sel_second')
        with cfc:
            ffr_third  = st.number_input('Third', value=int(c_def), step=1, format='%d', key='ffr_sel_third')
        with cfd:
            ffr_fourth = st.number_input('Fourth', value=int(d_def), step=1, format='%d', key='ffr_sel_fourth')

        st.markdown("### Actions")
        ac1, ac2 = st.columns(2)
        with ac1:
            if st.button('Calculate FFR Approximates / WillPays', key='btn_ffr_approxs'):
                try:
                    gross = float(st.session_state.get('gross_sales', 0.0))
                    refunds_val = float(st.session_state.get('refunds', 0.0))
                    jackpot_val = float(st.session_state.get('jackpot_in', 0.0))
                    net = net_pool(gross, refunds_val, jackpot_val, rules.commission)
                    st.session_state['net'] = net
                    u = ffr_units_from_spread(first_sp, second_sp, third_sp, fourth_sp, pool_total,
                                              int(ffr_first), int(ffr_second), int(ffr_third), int(ffr_fourth))
                    approx = ffr_approximate_dividend(net, float(u), rules)
                    st.success(f"FFR Approx/WillPay for {ffr_first}-{ffr_second}-{ffr_third}-{ffr_fourth}: ${approx}")
                    try:
                        if float(approx) == 0.0:
                            st.caption(f"[debug] Net=${net:,.2f}, Units={float(u):,.4f} (per $1 units)")
                    except Exception:
                        pass
                    ...
                    st.exception(e)
        with ac2:
            if st.button('Calculate FFR Declared Dividend', key='btn_ffr_declared'):
                try:
                    gross = float(st.session_state.get('gross_sales', 0.0))
                    refunds_val = float(st.session_state.get('refunds', 0.0))
                    jackpot_val = float(st.session_state.get('jackpot_in', 0.0))
                    ...
                    u = ffr_units_from_spread(first_sp, second_sp, third_sp, fourth_sp, pool_total,
                                              int(ffr_first), int(ffr_second), int(ffr_third), int(ffr_fourth))
                    declared = ffr_declared_dividend(net, float(u), rules)
                    st.success(f"Declared FFR dividend for {ffr_first}-{ffr_second}-{ffr_third}-{ffr_fourth}: ${declared}")
                except Exception as e:
                    st.exception(e)
        # ---- All FFR WillPays (Top N) ----
        st.markdown("### All FFR WillPays")
        with st.expander("Show FFR units formula (optional)"):
            st.markdown("""Units ≈ **Total** × (F[a]/ΣF) × (S[b]/ΣS) × (T[c]/ΣT) × (Q[d]/ΣQ)
            
            This normalizes each column by its own total and scales by the pool total; results are shown per $1 (10¢ units ×10).
            """)

        top_n = st.number_input("Show top N WillPays", value=200, min_value=1, max_value=5000, step=50, key="ffr_topn")
        if st.button("Generate All FFR WillPays (Top N)", key="btn_ffr_all"):
            try:
                import pandas as pd
                ...
                totalD = Decimal(str(pool_total)) if pool_total is not None else Decimal(0)
                # Build candidate lists with non-zero exposure
                candA = [r for r,u in (first_sp or {}).items()  if float(u)  > 0]
                candB = [r for r,u in (second_sp or {}).items() if float(u) > 0]
                candC = [r for r,u in (third_sp or {}).items()  if float(u)  > 0]
                candD = [r for r,u in (fourth_sp or {}).items() if float(u) > 0]

                rows = []
                for a in candA:
                    Fa = Decimal(str(first_sp[a]))
                    for b in candB:
                        if b == a: continue
                        Sb = Decimal(str(second_sp[b]))
                        for c in candC:
                            if c in (a,b): continue
                            Tc = Decimal(str(third_sp[c]))
                            for d in candD:
                                if d in (a,b,c): continue
                                Qd = Decimal(str(fourth_sp[d]))

                                # Units ≈ F[a] * S[b] * T[c] * Q[d] / Total^3
                                u = ffr_units_from_spread(first_sp, second_sp, third_sp, fourth_sp, totalD, int(a), int(b), int(c), int(d))
                                if u <= 0:
                                    continue
                                approx = ffr_approximate_dividend(net, float(u), rules)
                                rows.append((a,b,c,d, float(u), float(approx)))

                if rows:
                    rows.sort(key=lambda r: r[-1], reverse=True)
                    rows = rows[:int(top_n)]
                    df = pd.DataFrame(rows, columns=["First","Second","Third","Fourth","Units","Approx ($1)"])
                    st.subheader("FFR WillPays (Top N)")
                    st.dataframe(df, use_container_width=True, hide_index=True)
                    st.download_button(
                        "Download FFR WillPays (CSV)",
                        df.to_csv(index=False).encode("utf-8"),
                        "ffr_willpays.csv", "text/csv", key="dl_ffr_willpays"
                    )
                    st.success(f"Generated {len(rows)} combinations (showing top {min(len(rows), int(top_n))}).")
                else:
                    st.info("No valid combinations with non-zero exposure.")
            except Exception as e:
                st.exception(e)
        # ---- End All FFR WillPays ----


except Exception as _ffr_err:
    st.error(f'FFR UI error: {_ffr_err}')
# =================== End FFR UI ===================
