...
def net_pool(gross_sales: float, refunds: float, jackpot_in: float, commission: float) -> float:
    """Net pool after commission, with commission applied only on sales (not jackpot).

    net = (gross_sales - refunds) * (1 - commission) + jackpot_in
    """
    sales = max((gross_sales or 0.0) - (refunds or 0.0), 0.0)
    net_sales = sales * (1.0 - max(commission or 0.0, 0.0))
    return net_sales + max(jackpot_in or 0.0, 0.0)
...
POOL_PRESETS = {
    ...
    "PLA": {"commission": 0.1450, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "display_dp": 2, "max_dividends": 1},
    ...
    "TRI":  {"commission": 0.2150, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "FFR":  {"commission": 0.2300, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "DUE":  {"commission": 0.1550, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    ...
    # Quaddies
    "EQD":  {"commission": 0.2050, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 8,  "display_dp": 2},
    "QAD":  {"commission": 0.2050, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 8,  "display_dp": 2},
...
COMMISSION_BY_JURISDICTION = {
    "VIC & NSW": {
        "WIN": 0.1450, "PLA": 0.1425, "QIN": 0.1750, "EXA": 0.2000, "TRI": 0.2150,
...
with st.sidebar:
    ...
    # Commission is locked to the configured jurisdiction + pool; no UI override.
            ...
            if df_tri is None or df_tri.empty:
                ...
                if isinstance(meta_tri, dict) and meta_tri.get('jackpot') is not None:
                    try:
                        ...
                        pass
                # Scratchings / refunds prefill from TRI collation
                try:
                    _tri_meta2 = parse_meta_from_collation(txt_tri)
                    sv = _tri_meta2.get('scratchings_value') if isinstance(_tri_meta2, dict) else None
                    if sv is not None:
                        st.session_state['refunds'] = float(sv or 0.0)
                except Exception:
                    pass
