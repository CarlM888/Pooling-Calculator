...
NP_MINDIV_CONDITIONAL = False        # when True: enable conditional min-div behaviours
NP_MINDIV_DISPLAY_ONE_FOR_MIN = False  # when True: display 1.00 on NP WIN/PLA when min-div case hits

def _np_should_display_one(pool: str, rules, computed_div: float, applied_floor: float) -> bool:
    """Return True when we should *display* 1.00 for NP WIN/PLA min-div cases."""
    try:
        if not NP_MINDIV_DISPLAY_ONE_FOR_MIN:
            return False
        if str(pool).upper() not in ("WIN", "PLA"):
            return False
        return abs(float(computed_div) - float(applied_floor)) < 1e-9
    except Exception:
        return False

def _dp_round(val: float, dp: int) -> float:
    try:
        return float(f"{float(val):.{int(dp)}f}")
    except Exception:
        return float(val or 0.0)

# Per-pool rounding override (dark launch). Leave empty to keep current behaviour.
ROUNDING_MODE_MAP = {}  # e.g., {"DD": "nearest"}; valid: "down"|"up"|"nearest"

def _apply_breakage_ex(value: float, rules, pool: str | None = None) -> float:
    """Wrapper that respects ROUNDING_MODE_MAP per pool; falls back to rules.break_mode."""
    eff_mode = getattr(rules, 'break_mode', 'down')
    try:
        if pool:
            eff_mode = ROUNDING_MODE_MAP.get(str(pool).upper(), eff_mode)
    except Exception:
        pass
    step = getattr(rules, 'break_step', 0.10)
    return _apply_breakage(value, step, eff_mode)
...
# === EXA (Exacta) Parser & Helpers ===
def parse_exa_collation_text(text: str, scale: float = 1.0):
    """Parse an EXA collation dump in matrix form.

    Format (for N runners) is usually printed as a horizontal matrix per first runner, e.g.:

        1- 2    0.000000  0.000000 11.000000 97.000000  4.000000 95.000000
        1- 8    5.000000
    ...
    Returns (df, meta) where df has columns: First, Second, Units.
    """  # noqa: E501
    import re as _re
    import pandas as pd

    txt = text or ""
    # Normalise some unicode glyphs/dashes but otherwise keep layout
    txt = (txt.replace("\r", "")
              .replace("\u00a0", " ")
              .replace("–", "-")
              .replace("—", "-")
    ...
    # Lines like "  1- 2  0.000000 0.000000 11.000000 97.000000 4.000000 95.000000"
    # represent a horizontal strip of combinations starting at second runner B0.
    # We map the numeric tokens across as (A,B0),(A,B0+1),...
    line_re  = _re.compile(r"(?m)^\s*(\d+)\s*-\s*(\d+)\s+(.+)$")
    float_re = _re.compile(r"[-+]?(?:\d{1,3}(?:,\d{3})*|\d+)(?:\.\d+)?")

    rows = []
    eff_scale = float(scale or 1.0)

    for a_str, b0_str, tail in line_re.findall(txt):
        a = int(a_str)
        b0 = int(b0_str)
        tokens = float_re.findall(tail)
        vals = []
        for tok in tokens:
            try:
    ...
    meta = {
        ...
        "layout": "pairs-horizontal",
    }
    return df, meta


def due_parse_exa_collation_text(text: str, scale: float = 1.0):
    """DUE uses the same matrix EXA collation format as EXA itself.

    This is kept as a thin wrapper for clarity and future divergence if needed.
    """
    return parse_exa_collation_text(text, scale)
# === End EXA Parser & Helpers ===
...
def net_pool_with_net_jackpot(gross_sales: float, refunds: float, jackpot_in_net: float, commission: float) -> float:
    """Net pool where JACKPOT is already net (i.e. do not re-commission it).

    This follows the pattern:
        net = (gross - refunds) * (1 - commission) + jackpot_in_net
    which matches VIC/NSW exacta jackpot handling where the CLD JACKPOT field is a net carry-over.
    """
    base_sales = max((gross_sales or 0.0) - (refunds or 0.0), 0.0)
    net_sales = base_sales * (1.0 - max(commission or 0.0, 0.0))
    return net_sales + max(jackpot_in_net or 0.0, 0.0)
...
POOL_PRESETS = {
    # Single-leg
    "WIN":  {"commission": 0.1450, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "PLA": {"commission": 0.1425, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "display_dp": 2, "max_dividends": 1},
    "QIN":  {"commission": 0.1750, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "EXA":  {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "TRI":  {"commission": 0.2100, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "FFR":  {"commission": 0.2250, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},
    "DUE":  {"commission": 0.1450, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 1,  "display_dp": 2},

    # Short multi-leg
    "RD":   {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 2,  "display_dp": 2},
    "DD":   {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "nearest", "max_dividends": 2,  "display_dp": 2},
    "TBL":  {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 3,  "display_dp": 2},

    # Quaddies
    "EQD":  {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 8,  "display_dp": 2},
    "QAD":  {"commission": 0.2000, "min_div": 1.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 8,  "display_dp": 2},

    # BIG6
    "BIG6": {"commission": 0.2500, "min_div": 0.00, "break_step": 0.10, "break_mode": "down", "max_dividends": 12, "display_dp": 2},
}
...
COMMISSION_BY_JURISDICTION = {
    "VIC & NSW": {
        "WIN": 0.1450, "PLA": 0.1425, "QIN": 0.1750, "EXA": 0.2000, "TRI": 0.2100,
        "DUE": 0.1450, "FFR": 0.2250, "RD": 0.2000, "DD": 0.2000, "TBL": 0.2000,
        "EQD": 0.2000, "QAD": 0.2000, "BIG6": 0.2500,
    },
    "QLD": {
...
with st.sidebar:
    ...
    commission = st.number_input("Commission (0–1)", key=f"num_commission_{pool}_{jurisdiction}", value=float(preset["commission"]), min_value=0.0, max_value=0.99, step=0.01, format="%.4f")
    min_div = st.number_input("Minimum Dividend ($)", key=f"num_minimum_dividend_{pool}_{jurisdiction}", value=float(preset["min_div"]), min_value=0.0, step=0.01, format="%.2f")
    break_step = st.number_input("Breakage Step ($)", key=f"num_breakage_step_{pool}_{jurisdiction}", value=float(preset["break_step"]), min_value=0.0, step=0.01, format="%.2f")
    break_mode = st.selectbox("Breakage Mode", ["down", "nearest", "up"], index=["down", "nearest", "up"].index(preset["break_mode"]))
    max_dividends = st.number_input("Max Dividends", key=f"num_max_dividends_{pool}_{jurisdiction}", value=int(preset["max_dividends"]), min_value=1, max_value=64, step=1)
    display_dp = st.number_input("Display Decimals", key=f"num_display_decimals_{pool}_{jurisdiction}", value=int(preset["display_dp"]), min_value=0, max_value=4, step=1)
...
# Single-race dead-heat expander
    ...
    if st.button("Calculate Single-leg Dividend", key="btn_win_single_div"):
        ...
        try:
            ...
            jackpot_val = float(st.session_state.get('jackpot_in', 0.0))
            net = net_pool_with_net_jackpot(gross, refunds_val, jackpot_val, rules.commission)
...
# ——— End single‑leg inputs ———
    ...
    with st.expander("📥 Import WIN Collation", expanded=False):
        ...
        # FIX: persist uploaded text across Streamlit Cloud reruns
        if up is not None:
            try:
                st.session_state["__upload_text__collation_upload_v6_1"] = (up.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__collation_upload_v6_1"] = (up.read() or b"").decode("utf-8", errors="ignore")
        default_text = st.session_state.get("__upload_text__collation_upload_v6_1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__collation_upload_v6_1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("collation_area_v6_1", "")
            __new_upload = st.session_state.get("__upload_text__collation_upload_v6_1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["collation_area_v6_1"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == 'QIN':
    ...
    with st.expander('📥 Import QIN Collation', expanded=False):
        ...
        if up_qin is not None:
            try:
                st.session_state["__upload_text__qin_collation_upload_v1"] = (up_qin.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_qin.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__qin_collation_upload_v1"] = (up_qin.read() or b"").decode("utf-8", errors="ignore")
        default_qin_text = st.session_state.get("__upload_text__qin_collation_upload_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__qin_collation_upload_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("qin_collation_area_v1", "")
            __new_upload = st.session_state.get("__upload_text__qin_collation_upload_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["qin_collation_area_v1"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == 'PLA':
    ...
    with st.expander('📥 Import PLA Collation', expanded=False):
        ...
        if up_pla is not None:
            try:
                st.session_state["__upload_text__pla_collation_upload_v1"] = (up_pla.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_pla.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__pla_collation_upload_v1"] = (up_pla.read() or b"").decode("utf-8", errors="ignore")
        default_pla_text = st.session_state.get("__upload_text__pla_collation_upload_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__pla_collation_upload_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("pla_collation_area_v1", "")
            __new_upload = st.session_state.get("__upload_text__pla_collation_upload_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["pla_collation_area_v1"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == "FFR":
    with st.expander("🧾 Transaction-based FFR (scan sells)", expanded=False):
        ...
        if up_tx is not None:
            try:
                st.session_state["__upload_text__ffr_tx_upload_v6"] = (up_tx.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_tx.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__ffr_tx_upload_v6"] = (up_tx.read() or b"").decode("utf-8", errors="ignore")
        default_tx = st.session_state.get("__upload_text__ffr_tx_upload_v6", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__ffr_tx_upload_v6"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("ffr_tx_area_v6", "")
            __new_upload = st.session_state.get("__upload_text__ffr_tx_upload_v6", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["ffr_tx_area_v6"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == 'EQD':
    with st.expander("🧾 Transaction-based EQD (scan sells)", expanded=False):
            ...
            if up_tx is not None:
                try:
                    st.session_state["__upload_text__eqd_tx_upload_v6"] = (up_tx.getvalue() or b"").decode("utf-8", errors="ignore")
                except Exception:
                    try:
                        up_tx.seek(0)
                    except Exception:
                        pass
                    st.session_state["__upload_text__eqd_tx_upload_v6"] = (up_tx.read() or b"").decode("utf-8", errors="ignore")
            default_tx = st.session_state.get("__upload_text__eqd_tx_upload_v6", "")
            # Prime text area from upload on first load / when file changes
            try:
                __last_key = "__last_upload_text__eqd_tx_upload_v6"
                __prior_upload = st.session_state.get(__last_key, "")
                __cur_area = st.session_state.get("eqd_tx_area_v6", "")
                __new_upload = st.session_state.get("__upload_text__eqd_tx_upload_v6", "")
                if __cur_area == "" or __cur_area == __prior_upload:
                    st.session_state["eqd_tx_area_v6"] = __new_upload
                st.session_state[__last_key] = __new_upload
            except Exception:
                pass
...
if pool == 'TRI':
    ...
    with st.expander('📥 Import TRI Collation', expanded=False):
        ...
        if up_tri is not None:
            try:
                st.session_state["__upload_text__tri_collation_upload_v1"] = (up_tri.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_tri.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__tri_collation_upload_v1"] = (up_tri.read() or b"").decode("utf-8", errors="ignore")
        default_tri_text = st.session_state.get("__upload_text__tri_collation_upload_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__tri_collation_upload_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("tri_collation_area_v1", "")
            __new_upload = st.session_state.get("__upload_text__tri_collation_upload_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["tri_collation_area_v1"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
        ...
        if st.button('Parse & load TRI triples', key='btn_parse_tri_v1'):
            ...
            if df_tri is None or df_tri.empty:
                ...
                # NEW: Scratchings / refunds from CLD via generic helper
                try:
                    meta_extra = parse_meta_from_collation(txt_tri or "")
                    if isinstance(meta_extra, dict):
                        _rf = meta_extra.get('scratchings_value')
                        if _rf not in (None, ""):
                            _rf = float(_rf)
                            st.session_state['__set__refunds__'] = _rf
                            st.session_state['__set__tri_refunds__'] = _rf
                except Exception:
                    pass
    ...
    if pool == 'EXA':
        ...
        with st.expander('📥 Import EXA Collation', expanded=False):
            ...
            if up_exa_file is not None:
                try:
                    st.session_state['__upload_text__exa_file_uploader_v1'] = (up_exa_file.getvalue() or b'').decode('utf-8', errors='ignore')
                except Exception:
                    try:
                        up_exa_file.seek(0)
                    except Exception:
                        pass
                    st.session_state['__upload_text__exa_file_uploader_v1'] = (up_exa_file.getvalue() or b'').decode('utf-8', errors='ignore')
            _exa_text_from_file = st.session_state.get('__upload_text__exa_file_uploader_v1', '')
            # Prime text area from upload on first load / when file changes
            try:
                __last_key = '__last_upload_text__exa_file_uploader_v1'
                __prior_upload = st.session_state.get(__last_key, '')
                __cur_area = st.session_state.get('txt_exa', '')
                __new_upload = st.session_state.get('__upload_text__exa_file_uploader_v1', '')
                if __cur_area == '' or __cur_area == __prior_upload:
                    st.session_state['txt_exa'] = __new_upload
                st.session_state[__last_key] = __new_upload
            except Exception:
                pass
            ...
            with colx2:
                ...
                if st.button('Parse & load EXA pairs', key='btn_parse_exa'):
                    ...
                    # Schedule totals / jackpot / refunds into generic inputs (no direct widget writes)
                    try:
                        import re
                        total = (meta_exa or {}).get('total')
                        jackpot = (meta_exa or {}).get('jackpot')

                        # Derive SCRATCHINGS (refunds) using the generic helper
                        scratchings_val = None
                        try:
                            meta_extra = parse_meta_from_collation(txt_exa or "")
                            if isinstance(meta_extra, dict):
                                scratchings_val = meta_extra.get('scratchings_value')
                        except Exception:
                            scratchings_val = None
                        ...
                        if jackpot is None:
                            ...
                            if m:
                                jackpot = float(m.group(1).replace(',', ''))

                        # Push into session_state using the same pattern as other pools
                        ...
                        if scratchings_val is not None:
                            try:
                                rf = float(scratchings_val)
                                # generic refunds used by the single‑leg inputs
                                st.session_state['refunds'] = rf
                                # EXA‑specific mirror so the EXA Refunds box shows the same
                                st.session_state['exa_refunds'] = rf
                            except Exception:
                                pass
                    except Exception:
                        pass
...
# DUET-specific net helper: apply commission to (gross - refunds), add jackpot,
# then split equally across the number of winning pairs (default 3).
def due_net_pool_per_pair(gross_sales: float, refunds: float, jackpot_in: float, commission: float, winners: int = 3) -> float:
    try:
        g = float(gross_sales or 0.0)
        r = float(refunds or 0.0)
        j = float(jackpot_in or 0.0)
        c = float(commission or 0.0)
    except Exception:
        # Fallback if floats fail for some reason
        g = gross_sales or 0.0
        r = refunds or 0.0
        j = jackpot_in or 0.0
        c = commission or 0.0
    base = max(g - r, 0.0)
    # Apply commission only to sales (gross - refunds)
    net_sales = base * (1.0 - max(min(c, 0.99), 0.0))
    # Jackpot is added on top of net sales (no commission on jackpot)
    win = max(int(winners or 1), 1)
    return (net_sales + max(j, 0.0)) / float(win)

def due_declared_dividend(net_per_pair: float, pair_units: float, rules: 'PoolRules') -> float:
    """Declared DUET dividend per $1 stake for a single pair.

    `net_per_pair` is the DUET net pool already split equally across the
    number of winning pairs (usually 3 for the three combinations among
    the first three placegetters).
    """
    if pair_units <= 0:
        return 0.0
    raw = float(net_per_pair) / float(pair_units)
    div = _apply_breakage(raw, rules.break_step, rules.break_mode)
    div = max(div, rules.min_div)
    return _format_display(div, rules.display_dp)
...
if pool == 'DUE':
    ...
    with st.expander('📥 Import DUE Collation (EXA format)', expanded=False):
        ...
        if up_due_file is not None:
            try:
                st.session_state['__upload_text__due_file_uploader_v1'] = (up_due_file.getvalue() or b'').decode('utf-8', errors='ignore')
            except Exception:
                try:
                    up_due_file.seek(0)
                except Exception:
                    pass
                st.session_state['__upload_text__due_file_uploader_v1'] = (up_due_file.getvalue() or b'').decode('utf-8', errors='ignore')
        _due_text_from_file = st.session_state.get('__upload_text__due_file_uploader_v1', '')
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = '__last_upload_text__due_file_uploader_v1'
            __prior_upload = st.session_state.get(__last_key, '')
            __cur_area = st.session_state.get('txt_due', '')
            __new_upload = st.session_state.get('__upload_text__due_file_uploader_v1', '')
            if __cur_area == '' or __cur_area == __prior_upload:
                st.session_state['txt_due'] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
    ...
    if st.button('Calculate DUE Approximates', key='btn_due_approx'):
        try:
            ...
            refunds_val = float(st.session_state.get('refunds', 0.0))
            jackpot_val = float(st.session_state.get('jackpot_in', 0.0))
            net = due_net_pool_per_pair(gross, refunds_val, jackpot_val, rules.commission, winners=3)
            st.session_state['net'] = net
            approxs_due = {}
...
if pool == 'RD':
    ...
    with st.expander("📥 Import RD Collation (EXA format)"):
        ...
        if up_rd is not None:
            try:
                st.session_state["__upload_text__rd_file_uploader_v1"] = (up_rd.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_rd.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__rd_file_uploader_v1"] = (up_rd.read() or b"").decode("utf-8", errors="ignore")
        _rd_text_from_file = st.session_state.get("__upload_text__rd_file_uploader_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__rd_file_uploader_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("txt_rd", "")
            __new_upload = st.session_state.get("__upload_text__rd_file_uploader_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["txt_rd"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == 'DD':
    ...
    with st.expander("📥 Import DD Collation (EXA format)"):
        ...
        if up_dd is not None:
            try:
                st.session_state["__upload_text__dd_file_uploader_v1"] = (up_dd.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_dd.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__dd_file_uploader_v1"] = (up_dd.read() or b"").decode("utf-8", errors="ignore")
        _dd_text_from_file = st.session_state.get("__upload_text__dd_file_uploader_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__dd_file_uploader_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("txt_dd", "")
            __new_upload = st.session_state.get("__upload_text__dd_file_uploader_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["txt_dd"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == 'TBL':
    ...
    with st.expander("📥 Import TBL Collation (EXA format)"):
        ...
        if up_tbl is not None:
            try:
                st.session_state["__upload_text__tbl_file_uploader_v1"] = (up_tbl.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_tbl.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__tbl_file_uploader_v1"] = (up_tbl.read() or b"").decode("utf-8", errors="ignore")
        _tbl_text_from_file = st.session_state.get("__upload_text__tbl_file_uploader_v1", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__tbl_file_uploader_v1"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("txt_tbl", "")
            __new_upload = st.session_state.get("__upload_text__tbl_file_uploader_v1", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["txt_tbl"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
...
if pool == "QAD":
    ...
    def due_declared_dividend(net: float, pair_units: float, rules: 'PoolRules') -> float:
        if pair_units <= 0:
            return 0.0
        raw = float(net) / float(pair_units)
        div = _apply_breakage_ex(raw, rules, "DUE")
        div = max(div, rules.min_div)
        return _format_display(div, rules.display_dp)
    ...
    # ========================= DUE (Duet) — using EXA collation format =========================
    def due_declared_dividend(net: float, pair_units: float, rules: 'PoolRules') -> float:
        # Duet: use full net (no 3-way split), QIN-style declared logic
        if pair_units <= 0:
            return 0.0
        per_pair_net = float(net)
...
if pool == "BIG6":
    ...
    with st.expander("📥 Import BG6 Collation (POOL TOTALS → Gross Sales)", expanded=False):
        ...
        # FIX: persist uploaded text across Streamlit Cloud reruns
        if up_bg6 is not None:
            try:
                st.session_state["__upload_text__bg6_collation_upload_strict"] = (up_bg6.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_bg6.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__bg6_collation_upload_strict"] = (up_bg6.read() or b"").decode("utf-8", errors="ignore")
        default_bg6_text = st.session_state.get("__upload_text__bg6_collation_upload_strict", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__bg6_collation_upload_strict"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("bg6_collation_area_strict", "")
            __new_upload = st.session_state.get("__upload_text__bg6_collation_upload_strict", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["bg6_collation_area_strict"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
    ...
    with st.expander("📥 Import Multi‑Leg Approximates (MUL)", expanded=False):
        ...
        if up_mul is not None:
            try:
                st.session_state["__upload_text__bg6_mul_upload_strict"] = (up_mul.getvalue() or b"").decode("utf-8", errors="ignore")
            except Exception:
                try:
                    up_mul.seek(0)
                except Exception:
                    pass
                st.session_state["__upload_text__bg6_mul_upload_strict"] = (up_mul.read() or b"").decode("utf-8", errors="ignore")
        default_mul = st.session_state.get("__upload_text__bg6_mul_upload_strict", "")
        # Prime text area from upload on first load / when file changes
        try:
            __last_key = "__last_upload_text__bg6_mul_upload_strict"
            __prior_upload = st.session_state.get(__last_key, "")
            __cur_area = st.session_state.get("bg6_mul_area_strict", "")
            __new_upload = st.session_state.get("__upload_text__bg6_mul_upload_strict", "")
            if __cur_area == "" or __cur_area == __prior_upload:
                st.session_state["bg6_mul_area_strict"] = __new_upload
            st.session_state[__last_key] = __new_upload
        except Exception:
            pass
    ...
    with st.expander("Consolation (5 of 6) options", expanded=False):
        ...
        consol_pct = st.number_input("Consolation pool percentage of Final Net Pool (0–100)", min_value=0.0, max_value=100.0, step=0.5, value=20.0, key="bg6_consol_pct_strict")
        u5_override = st.number_input(
            "Total 5-of-6 winner units (optional override)",
            min_value=0.0,
            step=0.1,
            value=0.0,
            key="bg6_u5_units_override",
        )
    ...
    if st.button("Calculate BIG6 Declared Dividend(s)", key="btn_bg6_declared_divs_strict"):
        ...
        if not last_nums:
            ...
            # BIG6 semantics:
            # - Major dividend is paid from the full net pool (after commission and jackpot semantics)
            # - Consolation (5 of 6) dividend is paid from a separate percentage of the same net pool.
            #   This percentage does not reduce the major pool – we just model the official product payout.
            major_pool = net  # use full net for the major dividend
            consol_pool = net * (p / 100.0)

            winners_units_used = sum(float(spread_units.get(wk, 0.0) or 0.0) for wk in winners_keys)
            st.info(
                f"Major Pool: ${major_pool:,.2f}  •  Consolation Pool: ${consol_pool:,.2f}  •  "
                f"Winners' Units used: {winners_units_used:.4f}  •  Jackpot mode: {sem}  •  "
                f"Jackpot used in Major: ${jackpot_effective:,.2f}  •  Jackpot out: ${jackpot_out_amount:,.2f}"
            )

            # Major declared dividends per $1
            divs_major = dividends_from_spread(
                major_pool,
                winners_keys,
                spread_units,
                rules,
                declare_per_winner=True,
            )

            # 5‑of‑6 (supplementary) dividend denominator
            # Prefer explicit override (from MUL 5‑of‑6 units). If not provided, assume 1.0 unit
            # so the consolation dividend is simply ConsolationPool / 1.0 in the common single‑winner case.
            try:
                U5_override = float(st.session_state.get("bg6_u5_units_override", 0.0) or 0.0)
            except Exception:
                U5_override = 0.0

            if enable_consol and consol_pool > 0.0:
                denom = U5_override if U5_override > 0.0 else 1.0
            else:
                denom = 0.0

            out_rows = [("Major — " + str(k), float(amt)) for k, amt in divs_major.items()]
            if enable_consol and consol_pool > 0.0 and denom > 0.0:
                out_rows.append(("Consolation (5 of 6)", float(consol_pool) / float(denom)))
            ...
            st.download_button(
                "Download BIG6 Declared Dividends (CSV)",
                df_out.to_csv(index=False).encode("utf-8"),
                "big6_declared_dividends.csv",
                "text/csv",
                key="dl_bg6_declared_divs",
            )
...
def _qad_declared_from_tx_with_units(tx_text: str, units: float, rules):
    ...
    try:
        div = _apply_breakage_ex(div, rules, "QAD")
        div = max(div, rules.min_div)
...
# --- DUET helpers appended ---
def due_declared_for_placers(net: float, pair_units_map: dict[tuple[int,int], float], placers: tuple[int,int,int], rules: "PoolRules") -> dict[tuple[int,int], float]:
    """
    Compute the three DUET declared dividends for the three pairs among the top-3 placers.
    - net:      DUET net per winning pair (sales after commission + jackpot, already split by winners)
    - pair_units_map: {(a,b): units} where (a,b) are runner numbers with a<b
    - placers:  (first, second, third) runner numbers (order doesn't matter)
    - rules:    PoolRules for DUE (commission/min_div/breakage/display_dp)
    Returns: {(a,b): declared_div, ...} for the three pairs.
    """
    if not placers or len(placers) != 3:
        return {}
    a,b,c = sorted(int(x) for x in placers)
    pairs = [(a,b), (a,c), (b,c)]
    out = {}
    for p in pairs:
        key = tuple(sorted(p))
        units = float(pair_units_map.get(key, 0.0))
        try:
            div = due_declared_dividend(net, units, rules)
        except NameError:
            # fallback to QIN formula if due_declared_dividend isn't defined
            raw = 0.0 if units <= 0 else float(net) / float(units)
            from math import floor, ceil
            step = float(getattr(rules, "break_step", 0.10) or 0.10)
            mode = str(getattr(rules, "break_mode", "down") or "down")
            scale = 1.0 / step
            if mode == "down":
                val = floor(raw * scale) / scale
            elif mode == "up":
                val = ceil(raw * scale) / scale
            else:
                val = round(raw * scale) / scale
            min_div = float(getattr(rules, "min_div", 1.00) or 1.00)
            if val < min_div:
                val = min_div
            dp = int(getattr(rules, "display_dp", 2) or 2)
            try:
                div = float(f"{val:.{dp}f}")
            except Exception:
                div = float(val)
        out[key] = div
    return out