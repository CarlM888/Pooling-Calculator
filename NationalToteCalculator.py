...
def net_pool(gross_sales: float, refunds: float, jackpot_in: float, commission: float) -> float:
    """Compute net pool assuming jackpot_in is already *net*.

    - Commission is applied only to today's sales (gross_sales - refunds).
    - jackpot_in is treated as net-of-commission and added afterwards.
    """
    # Net from today's betting
    sales_base = max((gross_sales or 0.0) - (refunds or 0.0), 0.0)
    net_sales = sales_base * (1.0 - max(commission or 0.0, 0.0))

    # Jackpot is assumed to be net already; do not re-commission it.
    jp_net = max(jackpot_in or 0.0, 0.0)
    return net_sales + jp_net
...
def tbl_parse_exa_collation_text(text: str, scale: float = 1.0):
    """Parse TBL collation using the same triple logic as TRI.

    This reuses :func:`parse_tri_collation_text` so that:
    - Triples are parsed identically to TRI (First, Second, Third, Units)
    - Header fields TOTAL / JACKPOT are interpreted the same way
      (preferring JACKPOT/JACKPOT IN and falling back to SEEDED JACKPOT)
    The returned meta keeps the old keys num_runners_leg1/2/3 for UI
    display purposes, but all three legs share the same runner count
    where available.
    """
    df, meta_tri = parse_tri_collation_text(text, scale=scale)
    meta = {
        "total": meta_tri.get("total"),
        "jackpot": meta_tri.get("jackpot"),
        "num_runners_leg1": meta_tri.get("num_runners"),
        "num_runners_leg2": meta_tri.get("num_runners"),
        "num_runners_leg3": meta_tri.get("num_runners"),
    }
    return df, meta
...
def due_parse_exa_collation_text(text: str, scale: float = 1.0):
    ...
    # Match numbers with optional thousands separators, e.g. 2,129.642856
