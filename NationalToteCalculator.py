...
if pool == 'WIN':
    ...
    if st.button("Calculate Single-leg Dividend", key="btn_win_single_div"):
        try:
            ...
            st.session_state["single"] = single_pool_dividend(net, float(st.session_state.get('pla_single_leg_units', 0.0) if 'single_leg_units' in locals() else 0.0), rules, pool=pool, place_winners=pw)
        except Exception as e:
            st.exception(e)
