    ...
    if m_sets:
        ...
        try:
            if combs is not None and percent is not None:
                u_per = percent/100.0 if percent > 1.5 else percent
                tk.lines = int(combs)
...
def _eqd_parse_transactions(text: str) -> dict:
    ...
    # Gross from SUB TOTALS or POOL TOTALS (QAD/EQD)
...
def parse_collation_text(text: str, field_index: int = 1, scale: float = 1.0, force_horizontal: bool=False):
    ...
    # Decide layout
    horizontal = bool(force_horizontal) if 'force_horizontal' in locals() else False
...
def pla_declared_dividends_with_deficiency(net: float, winners_units: dict[int, float], place_winners: int, rules: "PoolRules") -> dict[int, str]:
    """Compute declared PLA dividends with 10c breakage (down) + min $1.00 + leftover top-ups."""
    winners = [int(k) for k in winners_units.keys()]
    if not winners or place_winners <= 0 or net <= 0:
        return {int(k): _format_display(0.0, getattr(rules, "display_dp", 2)) for k in winners_units}
    step = getattr(rules, "break_step", 0.10) or 0.10
    min_div = getattr(rules, "min_div", 1.00)
    dp = getattr(rules, "display_dp", 2)
    share = float(net) / float(place_winners)

    floored = {}
    for r in winners:
        u = float(winners_units.get(r, 0.0))
        if u <= 0.0:
            floored[r] = min_div
            continue
        raw = share / u
        if step > 0:
            val = math.floor(raw / step) * step
        else:
            val = raw
        floored[r] = max(val, min_div)

    payout = sum(floored[r] * float(winners_units.get(r, 0.0)) for r in winners)
    leftover = max(0.0, float(net) - payout)

    def bump_cost(rr: int) -> float:
        return step * float(winners_units.get(rr, 0.0))

    def candidates():
        cs = []
        for rr in winners:
            if float(winners_units.get(rr, 0.0)) <= 0.0:
                continue
            if abs(floored[rr] - min_div) < 1e-12:
                # don't top-up the $1.00 runner
                continue
            cs.append(rr)
        return cs
    ...
    while leftover > 1e-9:
        ...
        if not cs:
            break
        rr = min(cs, key=bump_cost)
        cost = bump_cost(rr)
        if cost <= 0.0 or leftover + 1e-12 < cost:
            break
        floored[rr] += step
        leftover -= cost
        it += 1
    ...
    return {r: _format_display(floored[r], dp) for r in winners}
...
if pool == 'WIN':
    ...
    with st.expander("Single‑leg pool inputs", expanded=False):
        ...
        with colA:
            _win_gross_sales = st.number_input("Gross Sales ($)", value=float(st.session_state.get("gross_sales", 0.00)), min_value=0.0, step=100.0, format="%.2f", key="win_gross_sales")
        with colB:
            _win_refunds = st.number_input("Refunds ($)", value=float(st.session_state.get("refunds", 0.00)), min_value=0.0, step=10.0, format="%.2f", key="win_refunds")
        with colC:
            _win_jackpot_in = st.number_input("Jackpot In ($)", value=float(st.session_state.get("jackpot_in", 0.00)), min_value=0.0, step=10.0, format="%.2f", key="win_jackpot_in")
        with colD:
            _win_units = st.number_input("Single-leg Winning Units (optional)", value=float(st.session_state.get("win_single_leg_units", 0.0)), min_value=0.0, step=1.0, format="%.2f", key="win_single_leg_units")
    if st.button("Calculate Single-leg Dividend", key="btn_win_single_div"):
        try:
            ...
            st.session_state["single"] = single_pool_dividend(net, float(st.session_state.get('win_single_leg_units', 0.0) if 'single_leg_units' in locals() else 0.0), rules, pool=pool, place_winners=pw)
        except Exception as e:
            st.exception(e)
...
if pool == 'PLA':
    ...
    with colD:
        single_leg_units = st.number_input("Single-leg Winning Units (optional)", value=0.0, min_value=0.0, step=1.0, format="%.2f", key='win_single_leg_units')
